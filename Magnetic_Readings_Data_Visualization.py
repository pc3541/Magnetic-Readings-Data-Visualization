...
from ipywidgets import widgets, Layout, FileUpload
...
print("Upload first .raw file (for gradient/deviation analysis):")
...
display(box2)
display(box3)
print("Upload second .raw file:")
uploader2 = widgets.FileUpload(accept='.raw', multiple=False)
display(uploader2)
print("Paste path to .xyz file (flight lines and times):")
xyz_path = widgets.Text(
    description='Path:',
    disabled=False)
display(xyz_path)
...
def run(_):
    with out:
        ...
        df = pd.read_csv(content, delim_whitespace=True, header=None)
        ...
        df2 = pd.read_csv(content2, delim_whitespace=True, header=None)
        ...
        path = str(xyz_path.value[1:-1])
        input_file3 = os.open(path, os.O_RDONLY)
        content3 = os.read(input_file3, sys.maxsize)
        os.close(input_file3)
        content3 = io.StringIO(content3.decode('utf-8'))
        molecule = pd.read_csv(content3, names=['Line', 'Aircraft', 'Flight', 'YYMMDD', 'Date', 'Time', 'DateU',
